...
router = APIRouter(prefix="/organizations/identities", tags=["Organization DID Resolution"])
...
def resolve_identity(
    did: str = Path(..., min_length=13, max_length=500), db: Session = Depends(get_db)
) -> DIDResolutionResponse:
    identity = resolve_organization_did(db, did)
    if identity is None:
